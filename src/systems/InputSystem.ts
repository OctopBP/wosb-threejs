import { InputComponent, MovementConfigComponent } from '../ecs/Component'
...
export class InputSystem extends System {
    ...
    private processPointerDirection(
        ...
        // Apply dead zone
        const pointerX =
            Math.abs(input.pointerX) > config.inputDeadZone
                ? input.pointerX
                : 0;
        const pointerY =
            Math.abs(input.pointerY) > config.inputDeadZone
                ? input.pointerY
                : 0;
