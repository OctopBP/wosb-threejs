...
export class RenderSystem extends System {
    ...
    private createPlaceholderShip(meshId: string): BABYLON.Mesh {
        ...
        const hull = BABYLON.MeshBuilder.CreateBox(`${meshId}_hull`, {
            width: 0.5,
            height: 0.2,
            depth: 1.5
        }, this.scene);

        const sail = BABYLON.MeshBuilder.CreateBox(`${meshId}_sail`, {
            width: 0.05,
            height: 0.8,
            depth: 0.6
        }, this.scene);
        ...
        // Create materials
        const hullMaterial = new BABYLON.StandardMaterial(`${meshId}_hull_material`, this.scene);
        hullMaterial.diffuseColor = new BABYLON.Color3(0.6, 0.4, 0.2); // Brown hull

        const sailMaterial = new BABYLON.StandardMaterial(`${meshId}_sail_material`, this.scene);
    ...
    private createEnemyMesh(meshId: string): BABYLON.Mesh {
        ...
        const hullMaterial = new BABYLON.StandardMaterial(`${meshId}_enemy_hull_material`, this.scene);
        hullMaterial.diffuseColor = new BABYLON.Color3(0.8, 0.2, 0.2); // Red hull

        const sailMaterial = new BABYLON.StandardMaterial(`${meshId}_enemy_sail_material`, this.scene);
        sailMaterial.diffuseColor = new BABYLON.Color3(0.3, 0.3, 0.3); // Dark sail

        // Apply to child meshes
        ship.getChildMeshes().forEach((child: BABYLON.AbstractMesh, index: number) => {
            if (index === 0) { // Hull
                (child as BABYLON.Mesh).material = hullMaterial;
            } else if (index === 1) { // Sail
    ...
    private createProjectileMesh(meshId: string): BABYLON.Mesh {
        const projectile = BABYLON.MeshBuilder.CreateSphere(`${meshId}`, {
            diameter: 0.1
        }, this.scene);

        const material = new BABYLON.StandardMaterial(`${meshId}_material`, this.scene);
